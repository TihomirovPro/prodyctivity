...
// icons for education_articles
gulp.task("sprite:svg:aside", async function () {
    return gulp
        .src(paths.views.iconsSvgAside + "/**/*.svg")
        .pipe(plumber())
        .pipe(svgmin({
            js2svg: {
                pretty: true
        ...
        .pipe(through2.obj(function (file, _encoding, cb) {
            ...
            this.push(file);
            gulp.src(__dirname + "/_sprite-svg-aside.scss")
                .pipe(consolidate("lodash", {
                    symbols: data
                }))
                .pipe(gulp.dest(paths.views.sassGen));
        ...
        .pipe(rename({
            basename: "sprite-aside"
        }))
        .pipe(gulp.dest(paths.images.dist));
});

gulp.task("sprite:svg", gulp.series("sprite:svg:mono", "sprite:svg:color", "sprite:svg:aside"));
